...
## Namangan Region Local Councils (Kengash) — Version 2.3

**Status:** AUTHORITATIVE. This version supersedes Version 2.2 in full. It makes a STRUCTURAL correction to Definition 2.1a after this exact category boundary (Category 2 / Category 12) broke for the third consecutive production run — see Section 0.1 for why this revision is different in kind from v2.1 and v2.2, and Global Principle G11 for the safeguard adopted to prevent a fourth recurrence.
...
### 0.1 Revision Log (v2.2 → v2.3) — Why This Fix Is Different in Kind

A production audit of a 1,150-post batch classified under v2.2 found **Category 2 = 0 posts**. This is the third consecutive production run in which the Category 2 / Category 12 boundary has required correction (v2.0→v2.1, v2.1→v2.2, now v2.2→v2.3), and this time the root cause is architectural, not a missing edge case.

**What went wrong:** Each of the first two fixes ADDED a clause to Definition 2.1a to correctly capture a specific type of content that was wrongly falling into Category 2. That was the right call each time, taken in isolation — but the cumulative effect, never checked at the time, was that Definition 2.1a's clauses became unbounded. Specifically:
- Clause (b) was written to say legal/legislative content qualifies "regardless of whether a specific deputy or official is named" and regardless of whether "the post carries a recognized recurring civic-education hashtag" — meaning ANY post mentioning ANY law, anywhere, about anything, satisfied it.
- Clause (a) (Named Council Actor present) did not require the actor to be doing anything distinctive — merely being listed as one of many attendees at a completely different institution's event was enough.

**Confirmed in the audited data:** posts explaining generic national seatbelt law, generic annual-leave-division labor rules, generic amusement-park safety rules, and a Ministry of Justice construction-cost instruction — none connected to this Kengash's own region, deputies, or locally-assigned law-monitoring mandate in any way — all satisfied 2.1a(b) and were classified Category 12. Most strikingly: coverage of the **Oliy Majlis Senate's own plenary session** (Uzbekistan's national parliament, not the local Kengash at all) was classified Category 12, with the classifier's own reasoning explicitly stating *"not the local Kengash"* — and it was still assigned Category 12, because local deputies were listed among the session's many attendees. `matched_rule: P13` (the gate that assigns Category 2) does not appear even once across the entire 1,150-post dataset: Node 13 had become mathematically unreachable.

**The fix is structural, not additive.** Rather than adding a clause (g), this revision adds a single, global **Local Nexus Requirement** that bounds ALL of clauses (a)–(f) at once: content must be meaningfully connected to *this specific monitored Kengash* — its own region, an actor of this Kengash actively doing something (not merely listed among attendees elsewhere), or a law this Kengash's own deputies are specifically assigned to monitor. Clauses (b) and (c) are also reworded to remove the unbounded language that caused this. See the revised Definition 2.1a, Category 2 section, and Decision Tree Node 11 below.

**New safeguard (Global Principle G11):** because this is the third occurrence of the same failure pattern, this revision also adds an explicit rule: no future amendment may broaden Definition 2.1a's scope without demonstrating, with concrete examples, that Category 2 remains satisfiable afterward.
...
**2.1a Council-Related Subject Matter** — This definition governs the Category 2 / Category 12 boundary specifically (Gates P11 and P13, Section 4). It is broader than, and independent of, Definition 2.1: a post can satisfy 2.1a without containing any Named Council Actor at all. A post satisfies "Council-Related Subject Matter" if and only if it satisfies the mandatory Local Nexus Requirement below AND its substantive content concerns ANY of the following:

  (a) A Named Council Actor (2.1) of **this specific monitored Kengash** is present, and the post's own primary subject is that actor's distinctive action or statement — not merely that actor's name appearing in a list of attendees at an event whose primary subject is a different institution.
  (b) The post explains, summarizes, announces, or references a law, legislative act, presidential decree, government resolution, legal right, or legal/regulatory procedure that is EITHER (i) explicitly tagged with this Kengash's own institutional law-monitoring hashtag (`#162_qonun`, `#162_ta_qonun`) or matches a specific law name in Appendix B's locally-assigned registry, OR (ii) explicitly connected to a Named Council Actor of this Kengash actively discussing or applying it (in which case clause (a) already applies). Generic national civic-education content about a law, regulation, or legal topic with NEITHER of these connections — e.g., a bare "did you know" post about traffic seatbelt rules, leave-division labor rules, or a ministry's technical instruction, with no tie to this Kengash's own law-monitoring mandate — does NOT satisfy this clause, regardless of which recurring hashtag it carries.
  (c) The post substantively concerns **this specific monitored Kengash's own** structure, powers, duties, function, or internal organizational activity (e.g., "Mahalliy Kengashlarning fuqarolar oldidagi vazifalari" describing the general duties of this channel's own institution-type, or an internal meeting evaluating this Kengash's own deputies' performance), even when referenced generically ("mahalliy kengash," "deputat") rather than via a specifically named individual. This clause does NOT extend to a different institution — a ministry, ministry-adjacent body, another region's council, or a national-level body such as the Oliy Majlis (Senate or Legislative Chamber) — merely because that institution is also, in some general sense, part of Uzbekistan's system of government or because deputies of this Kengash attended.
  (d) The post substantively concerns deputies' duties, responsibilities, ethics, or role in a generic, non-individually-named sense (e.g., `#deputat_vazifasi`, "deputat o'z vazifalarini vijdonan bajarishi kerak"), or explains how citizens can identify or contact their deputy (e.g., an E-Kengash portal promotion) — this is inherently about the deputy role as it applies to this Kengash's own channel and satisfies the Local Nexus Requirement without further test.
  (e) The post describes an investigative, informational, or civic-support activity conducted at the mahalla/community level, within this Kengash's own monitored region, on a governance-, social-protection-, or public-service-adjacent topic, even if no specific named actor performed it (e.g., an unattributed report of a mahalla polyclinic visit or mahalla banker study).
  (f) The post reports external or national-level recognition, praise, or media coverage of a specific governance initiative, program, innovation, or achievement originating in this Kengash's own region or district — even when no law, council institution, or named deputy is mentioned, and even when the initiative itself is not literally a Kengash program (e.g., national recognition of a regional "aql markazi"/think-tank model, or presidential acknowledgment of a local experience being adopted elsewhere).

**MANDATORY LOCAL NEXUS REQUIREMENT (new in v2.3) — governs ALL of (a)–(f) above:** A post satisfies Council-Related Subject Matter only if its content is meaningfully connected to *this specific monitored Kengash* — its own region/district, an actor of this Kengash actively doing or saying something (not merely being listed as one of many attendees at a different institution's event), or a law this Kengash's own deputies are specifically assigned to monitor per Appendix B. A post FAILS this requirement, and is therefore NOT Council-Related Subject Matter regardless of which clause (a)–(f) it superficially resembles, if it is any of the following:
  - Coverage of a **different institution's** own session, decision, or activity (e.g., the Oliy Majlis Senate or Legislative Chamber, a ministry, the Cabinet of Ministers, another region's council) where this Kengash's deputies are, at most, passively named among many other attendees or participants, and are not themselves the post's primary subject.
  - **Generic national civic-education or legal-awareness content** with no reference to this specific region, channel, or any locally-assigned law-monitoring connection (see clause (b) above for the precise test).
  - Content that is **verbatim-duplicated across multiple monitored channels** with no channel-specific commentary, local actor, or local framing added — a strong signal (though not by itself automatically dispositive) that the content is centrally-syndicated national material rather than this Kengash's own institutional communication. Where duplication is present, classifiers must independently verify at least one of (a)–(f) is genuinely satisfied on the substantive merits before assigning Category 12; duplication alone is never sufficient to satisfy 2.1a, and its presence should prompt closer scrutiny, not looser scrutiny.

Content failing the Local Nexus Requirement defaults to Category 2 (per Gate P13), Template Post (Gate P12, Definition 2.9), or a more specific higher-priority category (Gates P1–P10) if independently applicable — never to Category 12 on the basis of 2.1a alone.

**Discriminating test for (f), to prevent conflation with genuine Category 2 content:** A post satisfies 2.1a(f) — and is therefore Category 12 — if it names a specific achievement, program, initiative, or experience and attributes it to this region/district. A post that is a generic calendar, religious, or national holiday greeting with no specific regional achievement, program, or initiative named at all (e.g., a plain New Year or Independence Day greeting) does NOT satisfy 2.1a(f) and remains eligible for Category 2 per Section 5's Category 2 specification. The test is whether a specific, named local accomplishment is the subject of the post — not whether the word "Namangan" or a date appears.

A post satisfying 2.1a (including the Local Nexus Requirement) is NEVER Category 2. It is Category 12, unless it independently matches a higher-priority gate (P1–P10) first — 2.1a is evaluated only at Gate P11, after all more specific categories have already been ruled out, and does not alter the boundaries of Categories 3–11 in any way.
 
Category 2 (Gate P13) is reserved exclusively for content that satisfies NONE of (a)–(f) above — i.e., content with no connection whatsoever to law, legislation, governance, or council/deputy institutional matters (for example, a purely patriotic or calendar greeting with no legal or institutional framing, or content entirely outside the civic/governmental domain).
...
**G11 (new in v2.3 — Non-Ratchet Principle).** The Category 2 / Category 12 boundary (Definition 2.1a) has required correction in three consecutive revisions (v2.0→v2.1, v2.1→v2.2, v2.2→v2.3), each time because a prior fix's added scope was never checked against the opposite failure mode. To prevent a fourth recurrence: **no future amendment may add scope to Definition 2.1a (a new clause, a broadened existing clause, or a loosened qualifier) without the amendment's author first demonstrating, using at least 3 concrete example posts drawn from real or realistic data, that Category 2 remains satisfiable after the change.** A proposed fix that cannot show live examples of content that would still correctly land in Category 2 afterward must be rejected or narrowed until it can. This check must be performed explicitly, in writing, as part of any future revision's Section 0.1 revision log — not merely assumed.
...
| 11 | Post satisfies the mandatory Local Nexus Requirement AND Council-Related Subject Matter (2.1a(a)-(f)), no prior gate matched — content must be meaningfully connected to THIS specific Kengash (its region, an actor of this Kengash actively acting, or a law this Kengash monitors); generic national content or coverage of a different institution fails this gate regardless of surface resemblance to (a)-(f) | 12 | P11 |
...
- The post fails Council-Related Subject Matter (Definition 2.1a) entirely — either it fails the mandatory Local Nexus Requirement outright, or none of 2.1a's conditions (a)–(f) are satisfied even after passing that requirement.
...
- Content entirely outside the civic/governmental domain (e.g., a weather notice with no connection to any council function)
- "Qonunchilikka ko'ra, konstruksiyasida xavfsizlik kamarlari nazarda tutilgan transportni boshqarayotgan haydovchi... xavfsizlik kamarini taqib olishlari shart" (a generic national seatbelt-law fact, `#Bilasizmi` tagged, no #162 tag, no Appendix B law match, no local actor) → **Category 2** (new in v2.3) — fails the Local Nexus Requirement; this and the next three examples were the exact posts that drove Category 2 to zero under v2.2 and motivated this revision.
- "Qonunchilikka ko'ra, xodim va ish beruvchi o'rtasidagi kelishuvga ko'ra har yilgi mehnat ta'tili qismlarga bo'linishi mumkin" (`#Mehnat_huquqlari #Bilasizmi`, generic annual-leave-division labor rule, no #162 tag, no local connection) → **Category 2** (new in v2.3), per the tightened Definition 2.1a(b) — contrast with the "Mehnat kodeksi" counterexample below, which IS tagged `#162_qonun`.
- "Har bir attraksion ishga tushirilishidan oldin kundalik texnik xizmat ko'rsatishdan o'tkazilishi kerak" (`#Bilasizmi`, generic amusement-park safety rule) → **Category 2** (new in v2.3).
- "Qurilish sohasida normativ hujjatlar qiymatini aniqlashning yagona tartibi joriy etildi" (a Ministry of Justice technical instruction on construction-document costing, `#Qonunchilikdagi_yangiliklar` tagged, no #162 tag, no local actor or Appendix B match) → **Category 2** (new in v2.3).
- "Bugun Oliy Majlis Senatining o'n yettinchi yalpi majlisi bo'lib o'tdi... shuningdek, Xalq deputatlari Namangan viloyati Kengashi deputatlari... ishtirok etdi" (coverage of the **national Senate's own plenary session**, with this Kengash's deputies merely listed among many other attendees, not the post's own primary subject) → **Category 2** (new in v2.3) — this is coverage of a different institution's own event; it fails the Local Nexus Requirement even though local deputies are named, per the corrected Definition 2.1a(a)/(c). See EDGE-2.4 below.
...
- "Hududlarni rivojlantirishda aql markazlarini yo'naltirish tajribasi Namangandan boshlanadi" (national recognition of a specific regional-development initiative originating in this region, no law/deputy/institution named) → **Category 12** per 2.1a(f).
**Edge cases:**
- **EDGE-2.1 (corrected in v2.3):** A legal explainer post mentions a law but names no deputy and describes no monitoring activity → **Category 12 ONLY IF** the law is tagged with `#162_qonun`/`#162_ta_qonun` or matches an Appendix B locally-assigned law by name (per the tightened Definition 2.1a(b)). If the law is generic national civic-education content with neither connection (e.g., traffic, generic labor, or generic technical/administrative rules with no tie to this Kengash's monitoring mandate) → **Category 2**. The v2.2 wording of this edge case ("regardless of deputy attribution... regardless of hashtag") was the specific defect that drove Category 2 to zero in production and is corrected here — see Section 0.1.
- **EDGE-2.2:** A repost that quotes the hokim strictly in his executive (non-deputy, non-council) capacity, describing a purely municipal or ceremonial matter with no legislative content and no connection to Kengash business at all (e.g., attending a ribbon-cutting for a facility wholly outside any Kengash-tracked topic) → Category 2 is correct — it satisfies none of 2.1a(a)–(f), and names no specific regional achievement or initiative (per the 2.1a(f) discriminating test).
- **EDGE-2.3:** A post carries a recurring channel civic-education hashtag (e.g., `#Bilasizmi`, `#162_qonun`) but the underlying content, on inspection, is not actually legal/legislative (e.g., the hashtag was applied to an unrelated announcement by mistake) → the hashtag is not itself dispositive; classify per the actual substantive content against 2.1a. If the actual content still fails the Local Nexus Requirement or all of 2.1a(a)–(f), Category 2 applies despite the hashtag's presence.
- **EDGE-2.4 (new in v2.3):** A post covers a session, decision, or activity of a **different institution** (the Oliy Majlis Senate or Legislative Chamber, a ministry, the Cabinet of Ministers, another region's council), and this Kengash's own deputies are named only as attendees among many others, not as the post's primary subject → **Category 2**, per the Local Nexus Requirement — this holds even though clause (a) (Named Council Actor present) might appear satisfied on a superficial reading. If, instead, a deputy of this Kengash is the post's own primary subject (e.g., "Deputat X Senat majlisida so'zga chiqib, mahalliy muammoni ko'tardi" — a deputy of this Kengash raising a local issue at the national session), clause (a) is genuinely satisfied and the post is Category 12.
---
...
- **EDGE-4.2:** A post investigates a topic not explicitly listed in the 24 Issues but structurally similar (e.g., internet connectivity in mahallas, farmer-household agricultural entrepreneurship support, or a general citizen-complaint/proposal review that does not map to a specific issue number) → **Category 12, specifically labeled "unmatched field investigation"** Appendix A remains a closed, fixed list (Definition 2.3) and is NOT expanded by this rule; the fix is to give this real, recurring pattern a distinct, trackable label inside Category 12 rather than merging it indistinguishably into "other." This label applies regardless of whether the hashtag `#24_masala` is present.
...
- **EDGE-4.4:** A regular deputy's mobile reception ("sayyor qabul"), voter meeting, or on-site review of citizen complaints/proposals that does not itself identify a single primary 24-Issue focus → **Category 12, labeled "unmatched field investigation"** per the same rule as EDGE-4.2. This is the regular-deputy counterpart to Category 3's broad catch-all for the three special officials (Definition 2.11); it does not create a new top-level category, but ensures this activity is visible and distinguishable within Category 12 rather than lost in "other."
---
...
- "Ijtimoiy sheriklik — hamkorlik va taraqqiyot omili" (explains what the law says, no deputy monitoring action) → **Category 12**, deterministically, per Definition 2.1a(b) — the distinction between "channel-authored" and "repost with no framing". Legal/legislative content is Category 12 either way
...
- Field investigation or constituency activity by a regular deputy or secretariat specialist that does not match any of the 24 Issues (Appendix A), and mobile-reception/voter-meeting activity by a regular deputy with no single primary 24-Issue focus, per EDGE-4.2 and EDGE-4.4 — labeled distinctly as "unmatched field investigation," not merged into a generic "other" bucket
- Reports of national or external recognition, praise, or media coverage of a specific achievement, program, or initiative originating in this Kengash's own region or district, per Definition 2.1a(f)
...
- "Deputati D.Sheraliyev ... fermer xo'jaliklarining ish jarayonlari o'rganildi" (a regular deputy's investigation of farmer households' work processes, matching no specific Appendix A issue) → Category 12, labeled "unmatched field investigation," per EDGE-4.2
- A regular deputy's "sayyor qabul" (mobile reception) and on-site review of citizens' complaints and proposals, with no single primary 24-Issue focus identifiable → Category 12, labeled "unmatched field investigation," per EDGE-4.4
...
NODE 11 — Council-Related Subject Matter Check (revised in v2.3)
  STEP 1 — Local Nexus Requirement (mandatory gate, checked first):
    Is the content meaningfully connected to THIS specific monitored
    Kengash — its own region/district, an actor of this Kengash actively
    doing/saying something (not merely listed among many attendees at a
    DIFFERENT institution's event, e.g. Oliy Majlis Senate, a ministry,
    another region's council), or a law this Kengash's deputies are
    specifically assigned to monitor (Appendix B)?
    Verbatim duplication of this text across multiple monitored channels
    with no local framing is a signal to scrutinize more closely, not less.
    → NO: FAILS 2.1a entirely — proceed to Node 12 (this post cannot be
      Category 12 via this Node regardless of a–f below).
    → YES: proceed to STEP 2.

  STEP 2 — Does the post also satisfy ANY of the following?
    (a) A Named Council Actor (Definition 2.1) of this Kengash is present,
        and is the post's own primary subject (not a passive attendee
        listing); OR
    (b) The post explains a law/legislative act/decree/resolution that is
        tagged with this Kengash's own #162_qonun/#162_ta_qonun monitoring
        hashtag OR matches an Appendix B locally-assigned law by name —
        NOT merely any generic national civic-education law content; OR
    (c) The post substantively concerns THIS Kengash's own structure,
        powers, duties, or internal organizational activity (not a
        different institution's), even referenced generically; OR
    ...
    (e) The post describes an investigative, informational, or
        civic-support activity at the mahalla/community level, within this
        Kengash's own region, on a governance-/public-service-adjacent
        topic, even if unattributed; OR
    (f) The post reports external/national recognition of a specific
        achievement or initiative originating in this Kengash's own
        region/district (per the 2.1a(f) discriminating test).
  → YES (any of a–f): ASSIGN CATEGORY 12. STOP.
  → NO (none of a–f apply): proceed to Node 12.
...
**V8.** Regression test suite: a fixed set of at least 50 representative posts (drawn from Section 5's Examples and Counterexamples across all categories) must be maintained as a golden dataset. This set MUST include, at minimum, the 11 corrected Category 12 examples (Section 0.1) - legal explainers without a named deputy, generic deputy-duty content, Kengash-institutional-function content, unattributed mahalla-level investigative content, unmatched field investigations by regular deputies, and regional-achievement/recognition content. Any classifier update (prompt change, model change) must reproduce the documented category for 100% of golden-dataset posts before deployment.
...
**V10.** Anomaly-flag verification: for every channel, `category_3_by_official`'s per-official share must be checked against Definition 2.4's 50% threshold as part of aggregation output validation, and any channel exceeding it must appear in that period's flagged-anomalies output. This check must run over the aggregator's actual output, not merely be documented as an intention — a production run in which 10 of 15 channels exceeded this threshold with zero flags produced (see pipeline_audit_report_run3.md, Finding P-2) demonstrates this check is not currently wired into the pipeline, and its absence is a validation gap this rule exists to close going forward.

**V11 (new in v2.3).** Category 2 floor check: if Category 2 equals **zero** for any reporting period with more than 200 total posts, this is treated as a strong signal of a recurrence of the defect described in Section 0.1 (Definition 2.1a scope creep), and triggers mandatory human review of a sample of that period's Category 12 assignments — specifically checking for generic national civic-education content and coverage of institutions other than this Kengash — before the report is finalized. V9 (the ceiling check) and V11 (the floor check) are companion rules: this category has now broken in both directions across its revision history (v2.0: too large; v2.2: exactly zero), and a single one-sided check was insufficient to catch the second failure mode. Both bounds must be checked on every run.
...
**Cross-document dependency:** `narrative_specification_v1.md`'s aggregation schema for `category_12_by_subtype` (Section 3.2 of that document) must be extended with a sixth subtype value, `unmatched_field_investigation`, to carry the new label introduced in this version's EDGE-4.2/EDGE-4.4 through to the aggregation and narrative layers. This is a required follow-up to this revision and is not automatically satisfied by adopting this document alone.
