...
## Namangan Region Local Councils (Kengash) — Version 3.0
...
| 7 | Post satisfies 162-Law Monitoring (2.4) with an individual actor (deputy, kotibiyat mudiri, or kotibiyat mutaxassisi) whose law-monitoring content is the post's primary subject (v3.0: see EDGE-6.3 — a special official's post does NOT match this gate if law-monitoring is only a secondary addendum within a primarily 24-Issue-style investigation; such posts fall through to Gate P9 instead). Committee Law Monitoring (2.5) is referenced here for completeness only — it is never operative at this gate in practice, since a committee actor is always already captured earlier at Gate P2. | 6 | P7 |
| 8 | Post has a Plan Execution Reference tied to the Council's own plan (2.8) | 11 | P8 |
| 9 | Actor is hokim, kengash raisi, or kotibiyat mudiri, performing any activity not captured by Gates P1–P8 (v3.0: including a post where law-monitoring content was present but secondary/incidental per EDGE-6.3/EDGE-3.3, since such a post does not actually match Gate P7) — including 24-Issue Investigations personally conducted by these officials — except the vacant-electoral-district substitute-duty scenario (EDGE-3.2, which routes to Gate P10) | 3 | P9 |
...
**V8.** Regression test suite: a fixed set of at least 50 representative posts (drawn from Section 5's Examples and Counterexamples across all categories) must be maintained as a golden dataset. This set MUST include, at minimum:
   - the legal explainers without a named deputy, generic deputy-duty content, Kengash-institutional-function content, and unattributed mahalla-level investigative content;
   - the two special-official-led, primarily-24-Issue posts with an incidental law-awareness addendum (Category 3, not Category 6);
   - the six kotibiyat-mutaxassisi/kotibiyat-mudiri-led standalone law-monitoring posts (Category 6, not Category 12);
   - the three standing-committee self-description/composition posts (Category 7, not Category 12);
   - the three daily "kotibiyat plan" posts (two to Category 4, one to Category 12) and one genuine weekly/monthly Council plan post (Category 10);
   any classifier update (prompt change, model change) must reproduce the documented category for 100% of golden-dataset posts before deployment.
...
**V10.** Targeted boundary spot-check: for each reporting period, draw a small manual sample from each of the four boundary types — (a) Category 6 posts led by a kotibiyat mudiri/mutaxassisi, (b) Category 3 vs. Category 6 posts led by a special official that also name a specific law, (c) Category 7 posts describing a committee's own composition/mandate rather than a discrete meeting, (d) any post framed as a "daily" plan — and confirm against Section 5's worked examples before finalizing the report. This targeted check exists because these four boundaries showed concentrated classifier variance in the Round-5 audit even though the spec text is now explicit; a full V8 golden-dataset pass alone may not catch every recurrence in live data at scale.

---

## 12. SUPERSESSION STATEMENT

This document (v3.0) is the sole and complete authoritative source for classification of Telegram posts in this monitoring pipeline.